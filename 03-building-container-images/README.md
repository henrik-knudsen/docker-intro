...
When building docker images, ensure to:
- Pass the correct path to the dockerfile you want to use, using `-f` option (default is ./Dockerfile)
- Pass the correct path of the build context you want to use, as first argument to `docker build` (i.e. `docker build .` if you want to build from current directory)
- Tag your images with a name, using the `-t` option (i.e. `docker build -t my-image .`)

Example, building first .NET image, standing in root directory of repo:

```bash
docker build -t api-dotnet -f 03-building-container-images/api-dotnet/Dockerfile.0 02-demo-web-application/api-dotnet
```
